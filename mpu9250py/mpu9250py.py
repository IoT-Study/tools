...
def pause_plot():
    #init garaph
    gm_acc = GraphManager(3)
    gm_gyr = GraphManager(3)
    gm_mag = GraphManager(3)
    gm_temp = GraphManager(1)

    fig, ([gm_acc.axis, gm_gyr.axis], [gm_mag.axis, gm_temp.axis])= plt.subplots(nrows=2,ncols=2, figsize=(12,8))
    
    gm_acc.setup("Acceleration (g)", ["x", "y", "z"], -2, 2, 0.001)
    gm_gyr.setup("Gyro (°/sec)", ["x", "y", "z"], -250, 250, 0.001*250)
    gm_mag.setup("Magnetic (uT)", ["x", "y", "z"], -100, 100, 1.0)
    gm_temp.setup("Temperature (°C)", ["t"], 0, 50, 0.1)
